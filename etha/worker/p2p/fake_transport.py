...
from etha.worker.p2p.messages_pb2 import Envelope, Query, WorkerState, RangeSet, Range
from etha.worker.p2p.p2p_transport_pb2 import Message, Empty
from etha.worker.p2p.p2p_transport_pb2_grpc import P2PTransportServicer, add_P2PTransportServicer_to_server


DEFAULT_STATE = WorkerState(
    datasets={
        's3://etha-mainnet-sia': RangeSet(ranges=[
            Range(begin=16143005, end=16149151)
        ])
    }
)
